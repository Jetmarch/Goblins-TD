...
namespace Game.Gameplay.Levels.Editor
{
    public class LevelGridConstructor : EditorWindow
    {
        ...
        {
            string path = EditorUtility.SaveFilePanel("Save Level Grid Config", "Assets", "NewLevelGridConfig", "asset");
            ...
            LevelGridConfig levelGridConfig = CreateInstance<LevelGridConfig>();
        ...
        {
            var path = EditorUtility.OpenFilePanel("Load Level Grid Config", "Assets", "asset");
        ...
        private void CreateLevelGrid()
        {
            ...
            for (int i = 0; i < _gridWidth; i++)
            {
                ...
                {
                    cells[i,j] = new LevelCell(i, j, _cellSize, LevelCellType.Buildable);
                }
            }
            _levelGrid = new LevelGrid(_gridWidth, _gridHeight, cells);
        }
        ...
        private void DrawGrid()
        {
            for (int y = 0; y < _levelGrid.Height; y++)
            {
                ...
                {
                    LevelCell cell = _levelGrid.Cells[x, y];
        ...
        private void ChangeCellType(LevelCell cell)
        {
            switch (cell.Type)
            {
                case LevelCellType.Buildable:
                    cell.Type = LevelCellType.Walkable;
                    break;
                case LevelCellType.Walkable:
                    cell.Type = LevelCellType.Buildable;
                    break;
            }
        }
